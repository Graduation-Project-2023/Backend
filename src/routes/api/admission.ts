...
server.post('/csv_upload', upload.single('csv'), (req:Request, res: Response, next: NextFunction) => {
    ...
    try{
        ...
        .then(async (jsonObj)=>{
            ...
            if (HD[3] != "englishName" || HD[4] != "arabicName" || HD[5] != "nationality" || HD[6] != "nationalId" || HD[7] != "gender" || HD[8] != "religion" || HD[9] != "birthDate" || HD[10] != "birthPlace" || HD[11] != "guardianName" || HD[12] != "address" || HD[13] != "contactPhone" || HD[14] != "homePhone")
            {
            ...
            for (let i = 0; i < jsonObj.length; i++) {
                if (!jsonObj[i].nationalId || jsonObj[i].nationalId.length != 14) {
                    ...
                    if (jsonObj[i].gender == "ذكر")
                        jsonObj[i].gender = "MALE"
                    else
                        jsonObj[i].gender = "FEMALE"
                    if (jsonObj[i].religion == "مسلم")
                        jsonObj[i].religion = "MUSLIM"
                    else
                        jsonObj[i].religion = "CHRISTIAN"
